...
class spline {
 ...
 public:
  ...
  int num_pts() const;
  double get_coeff(int n, int j) const;
};
...
int spline::num_pts() const { return m_a.size(); }

double spline::get_coeff(int n, int j) const {
  assert(n < static_cast<int>(m_x.size()));
  assert(j < 4);  // cubic spline
  switch (j) {
    case 0:
      return m_y[n];
    case 1:
