...
ErrorType LaneGenerator::GetLaneBySamplePoints(const vec_Vecf<LaneDim>& samples, Lane* lane) {
  ...
  for (int i = 1; i < (int)samples.size(); ++i) {
    ...
    d += std::hypot(dx, dy);
    para.push_back(d);
#ifdef DEBUG_LANE_SET
    std::cout << "Index: " << i << "\tpara: " << d << std::endl;
#endif
  }

#ifdef DEBUG_LANE_SET
  std::cout << "---------- end para - GetLaneBySamplePoints ----------\n";
#endif
...
ErrorType LaneGenerator::GetLaneBySampleFitting(const vec_Vecf<LaneDim>& samples,
                                                const std::vector<decimal_t>& para,
                                                const Eigen::ArrayXf& breaks,
                                                const decimal_t regulator, Lane* lane) {
  Spline<LaneDegree, LaneDim> spline;
  SplineGenerator<LaneDegree, LaneDim> spline_generator;

  if (spline_generator.GetQuinticSplineBySampleFitting(samples, para, breaks, regulator, &spline) !=
      kSuccess) {
